            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <!--main content-->
                        <form id="commentForm" action="{{ route('lession.store') }}"
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select name="courses_id" class="form-control" id="courses_id" onchange="onchange_course(this.value)">
                                        ...
                                        <option value="{{ $courses->id }}" @if(old('courses_id') == $courses->id){{ 'selected' }}@endif >{{ $courses->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <select name="chapter_id" class="form-control" id="chapter_id" readonly>
                                        <option value="">Select Chapter</option>
                                        @foreach($chapter as $chapters)
                                        <option value="{{ $chapters->id }}" @if(old('chapter_id') == $chapters->id){{ 'selected' }}@endif >{{ $chapters->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
@section('footer_scripts')
    ...
    <script>
        function onchange_course(course_id){
            $("#chapter_id").removeAttr('readonly');
            $.ajax({
